...
system.time({
  ####5km tiff####
  p2210 <- (al_list[[5]] - al_list[[1]])/al_list[[1]]
  ...
  write_sf(geo_sf,"./shape/5km_diff_rake_2210_rltv.shp")
  rm(geo_sf,mat5_diff)
})
...
write.csv(mat5_diff,"./shape/con_diff_rake_2210_rltv.csv",col.names = T,row.names = F)
system.time({
  ####dis tiff####
  ...
  write.csv(mat5_diff,"./shape/pro_diff_rake_2210_rltv.csv",col.names = T,row.names = F)
})
system.time({
  ####dis tiff####
  ...
  write.csv(mat5_diff,"./shape/dis_diff_rake_2210_rltv.csv",col.names = T,row.names = F)
})
rm(list=ls())
gc()