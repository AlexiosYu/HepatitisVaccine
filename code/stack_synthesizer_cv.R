...
stack_synthesizer_function <- function(mesh, ml_res,processed,model_names,fold_i) {
  time_a <- Sys.time()
  ...
  tmpY <- grouped_data$obs
  tmpY[grouped_data$nfolds==i]=NA
  inx <- which(grouped_data$nfolds==i)
  obs_na <- grouped_data$obs[grouped_data$nfolds==i]
  ##select submodel###
