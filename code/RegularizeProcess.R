...
data_process_function <- function(shp_sf,data,n,N){
  ...
  combine <- rbind(pred_csv[,cov.name],data[,cov.name])
  samp <- sample(1:nrow(combine),nrow(combine),replace=F)###randomize data
  combine <- combine[samp,]
  
  preProcess_range_model <- preProcess(combine[,cov.name], method='range') #scale the covariates
