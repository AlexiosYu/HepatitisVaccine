...
for (i in c(2011:2022)-2009) {
  ...
  colnames(mat) <- 1:1000
  write.csv(df.final,sprintf("./shape/rake/%s/%s_%s%s","D3","all",selected_year_index[i] + 2009,".csv"),row.names = F)
}
write.csv(mat,sprintf("./shape/rake/%s/%s_%s%s","D3","all","mat",".csv"),row.names = F)
