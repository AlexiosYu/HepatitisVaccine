...
for (i in c(13,2:5,7:9,11,12)) {
  ...
  cat("\n")
  xbar = parApply(cl,adjusted_pred,MARGIN=1,mean,na.rm=T)
  time2 <- Sys.time()
  cat(paste(time2,"mean"))
  cat("\n")
  s = parApply(cl,adjusted_pred,MARGIN=1,sd,na.rm=T)
  time3 <- Sys.time()
  cat(paste(time3,"sd"))
  cat("\n")
  qt <- parApply(cl,adjusted_pred, MARGIN=1, function(x) quantile(x,c(0.025,0.975),na.rm=T)) %>% t() %>% as.data.frame()
  time4 <- Sys.time()
  cat(paste(time4,"qt"))
  cat("\n")
  df.final <- data.frame(adjusted_tibs,
                         mean = xbar,
                         sd = s,
                         low = qt$`2.5%`,
                         up = qt$`97.5%`)
  ...
  stopCluster(cl)
  ncluster <- 5
  pred_dis <- rake_poly_function(ncluster, adjusted_pred,adjusted_tibs, c("con_id", "pro_id", "dis_id"),yr)
  gc()
  pred_pro <- rake_poly_function(ncluster, adjusted_pred,adjusted_tibs, c("con_id", "pro_id"),yr)
  gc()
  pred_con <- rake_poly_function(ncluster, adjusted_pred,adjusted_tibs, c("con_id"),yr)
  gc()
  time_3 <- Sys.time()
  sprintf("Time spent on process 2: \n") %>% cat()
  print(time_3-time1)
  write_name <- sapply(1:4, function(j){
    sprintf("./shape/rake/%s/%s_%s%s","D1",c("5km","dis","pro","con")[j],selected_year_index[i] + 2009,c(".shp",".csv",".csv",".csv")[j])
  })
